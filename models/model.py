...
class IDAUp(nn.Module):
    ...
    def __init__(self, out_dim, channel):
        ...
        self.out_dim = out_dim          # out_dim = 24 in all cases
        # print('IDAUp channel:', channel)

        # Down sampling
        # down sample from input channel to out_dim channel without changing in shape
...
class MobileNetV2(nn.Module):
    def __init__(self, width_mult=1.0, round_nearest=8,):
        ...
        input_channel = 32
        '''
            t: expansion ratio (c -> t*c channels)
            c: input channels 
            n: number of time to apply kernel
            s: stride
        '''
        ...
        # get the feature map from layer 1, 2, 4, 6
        self.feat_id = [1,2,4,6]
        # output channel after above layers
        self.feat_channel = []
        ...
        # print('Base input channel:', input_channel)
        ...
        for id, (t, c, n, s) in enumerate(inverted_residual_setting):
            ...
            # print('output channel {}:'.format(id), output_channel)
        ...
        # print('Base feature channel:', self.feat_channel)
...
class MobileNetSeg(BaseModel):
    def __init__(self, base_name, heads, head_conv=24, pretrained = True):      # was 24
        ...
        # output convs have 24 channels 
        self.dla_up = MobileNetUp(channels, out_dim=head_conv)

        for head in self.heads:
            # number of class per head (hm, wh, hm_offset, landmark)
            classes = self.heads[head]
            if head == 'hm':
                # len = number of classes, apply sigmoid to get prob of each class
                fc = nn.Sequential(
    ...
    # convert dict to list to support onxx format
    def forward(self, x):
        x = self.base(x)
        x = self.dla_up(x)
        ret = {}
